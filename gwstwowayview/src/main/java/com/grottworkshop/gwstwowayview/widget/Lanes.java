...
class Lanes {
    ...
    public TwoWayLayoutManager.Orientation getOrientation() {
        return (mIsVertical ? TwoWayLayoutManager.Orientation.VERTICAL : TwoWayLayoutManager.Orientation.HORIZONTAL);
    }
    ...
    public int pushChildFrame(Rect outRect, int lane, int margin, TwoWayLayoutManager.Direction direction) {
        final int delta;
    ...
    public void popChildFrame(Rect outRect, int lane, int margin, TwoWayLayoutManager.Direction direction) {
        final Rect laneRect = mLanes[lane];
        if (mIsVertical) {
            if (direction == TwoWayLayoutManager.Direction.END) {
    ...
    public void getChildFrame(Rect outRect, int childWidth, int childHeight, LaneInfo laneInfo,
                              TwoWayLayoutManager.Direction direction) {
        ...
        final int anchorLane =
                (direction == TwoWayLayoutManager.Direction.END ? laneInfo.anchorLane : laneInfo.startLane);
        final Rect anchorRect = mLanes[anchorLane];

        if (mIsVertical) {
            outRect.left = startRect.left;
            outRect.top =
                    (direction == TwoWayLayoutManager.Direction.END ? anchorRect.bottom : anchorRect.top - childHeight);
        } else {
            outRect.top = startRect.top;
            outRect.left =
                    (direction == TwoWayLayoutManager.Direction.END ? anchorRect.right : anchorRect.left - childWidth);
        }
    ...
    private int findLaneThatFitsSpan(int anchorLane, int laneSpan, TwoWayLayoutManager.Direction direction) {
    ...
    public void findLane(LaneInfo outInfo, int laneSpan, TwoWayLayoutManager.Direction direction) {
        outInfo.setUndefined();

        int targetEdge = (direction == TwoWayLayoutManager.Direction.END ? Integer.MAX_VALUE : Integer.MIN_VALUE);
        for (int l = 0; l < mLanes.length; l++) {
            final int laneEdge;
            if (mIsVertical) {
                laneEdge = (direction == TwoWayLayoutManager.Direction.END ? mLanes[l].bottom : mLanes[l].top);
            } else {
                laneEdge = (direction == TwoWayLayoutManager.Direction.END ? mLanes[l].right : mLanes[l].left);
            }
